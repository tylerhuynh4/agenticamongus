...
export class OpenRouterClient implements LlmClient {
  async complete(prompt: string, system?: string): Promise<LlmResult> {
    if (!config.openRouterApiKey) {
      throw new Error(
        "Missing OpenRouter configuration. Set OPENROUTER_API_KEY in your environment.",
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `OpenRouter request failed with status ${response.status}: ${errorText.trim()}`,
        );
      }
